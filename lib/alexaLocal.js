...
function alexaLocal(options) {
  ...
  connection.client.on('error', function(err) {
    ...
    switch (err.code) {
      case 5:
        // join on ' ' here because servers is an array of objects, but we expect it to only ever contain one value
        options.log(`ERROR: ( homebridge-alexa) Login to ${options.servers.map(x => x.host).join(' ')} failed, please validate your credentials in config.json and restart homebridge.`);
        connection.client.end({
          force: true
        });
        break;
      default:
        debug('ERROR: %s -> ', err.message, err.code);
    }
  });
}
